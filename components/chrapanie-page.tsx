...
export function ChrapaniePage() {
  const { t, i18n } = useTranslation()
  const lang = i18n.language === "en" ? "en" : "pl"
  const page = lang === "en" ? en.chrapaniePage : pl.chrapaniePage
  const e = page.apnea
      ...
      <section className="bg-white py-16">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="space-y-16">
            ...
            {/* ── Symptoms ── */}
            <div>
              <h3 className="text-balance mb-6 text-xl font-bold text-slate-900">{e.symptoms}</h3>
              <div className="grid gap-x-8 sm:grid-cols-2">
                {e.symptomsList.map((item: string) => (
            ...
            {/* ── Risk factors ── */}
            <div>
              <h3 className="text-balance mb-8 text-xl font-bold text-slate-900">{e.riskHeading}</h3>
              <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-4">
                {e.riskList.map((risk) => (
                  <div key={risk.title} className="border-l-2 border-[#EE3920] pl-5">
                    <h4 className="text-balance mb-2 font-semibold text-slate-900">{risk.title}</h4>
                    <p className="text-pretty text-sm leading-relaxed text-slate-600">{risk.body}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            {/* ── Diagnostic methods ── */}
            <div>
              <h3 className="text-balance mb-6 text-xl font-bold text-slate-900">{e.diagnostics}</h3>
              <div className="grid gap-x-8 sm:grid-cols-2">
                {e.diagnosticsList.map((item: string) => (
                  <div key={item} className="flex items-start gap-2.5 border-b border-slate-100 py-3">
                    <CheckCircle className="mt-0.5 h-4 w-4 shrink-0 text-[#EE3920]" />
                    <span className="text-sm text-slate-700">{item}</span>
                  </div>
            ...
            {/* ── Health risks ── */}
            <div>
              <h3 className="text-balance mb-6 text-xl font-bold text-slate-900">{e.dangers}</h3>
              <div className="grid gap-x-8 sm:grid-cols-2">
                {e.dangersList.map((item: string) => (
                  <div key={item} className="flex items-start gap-2.5 border-b border-slate-100 py-3">
            ...
            {/* ── WatchPAT 200 ── */}
            <div>
              <p className="text-pretty mb-2 text-xs font-semibold uppercase tracking-widest text-[#EE3920]">{e.watchpat.eyebrow}</p>
              <h3 className="text-balance mb-5 text-3xl font-bold text-slate-900">{e.watchpat.heading}</h3>
              <div className="grid gap-8 lg:grid-cols-[1fr_auto] lg:items-start">
                <div className="max-w-2xl">
                  <p className="text-pretty mb-4 text-base leading-relaxed text-slate-600">{e.watchpat.body1}</p>
                  <p className="text-pretty mb-4 text-base leading-relaxed text-slate-600">{e.watchpat.body2}</p>
                  <p className="text-pretty text-base leading-relaxed text-slate-600">{e.watchpat.body3}</p>
                </div>
                <div className="rounded-xl bg-slate-50 p-6 lg:max-w-sm">
                  <h4 className="text-balance mb-4 font-semibold text-slate-900">{e.watchpat.featuresHeading}</h4>
                  <div className="space-y-1.5">
                    {e.watchpat.features.map((item: string) => (
              ...
              <div className="mt-8 border-l-2 border-[#EE3920] pl-5">
                <h4 className="text-balance mb-2 font-semibold text-slate-900">{e.watchpat.driversHeading}</h4>
                <p className="text-pretty max-w-3xl text-sm leading-relaxed text-slate-600">{e.watchpat.driversBody}</p>
              </div>
            </div>

            {/* ── SomnoGuard ── */}
            <div>
              <p className="text-pretty mb-2 text-xs font-semibold uppercase tracking-widest text-[#EE3920]">{e.somnoguard.eyebrow}</p>
              <h3 className="text-balance mb-5 text-3xl font-bold text-slate-900">{e.somnoguard.heading}</h3>
              <p className="text-pretty max-w-3xl text-base leading-relaxed text-slate-600">{e.somnoguard.body}</p>
            </div>
